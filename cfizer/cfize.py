...
def process_large(filepath: str, group: DsGroup, title: str, shared: dict) -> str:
    ...
    if shared["verbose"]:
        log.append(
            f"{strftime('%H:%M:%S', localtime())} Process {os.getpid()}: process_large running on group "
    ...
    for i, ds in enumerate(processed):
        ...
        history = (
            ...
            + f": output file {op.basename(filepath)} processed for CF compliance using CFizer version {VERSION} (https://github.com/cemac/CFizer); configuration and source files listed in log file {shared['logfile']}"
        )
...
def cf_merge(group: DsGroup, shared: dict) -> str:
    ...
    try:
        ...
        update_group = (
            ...
            else {"processed": group.processed, "time_var": group.time_var}
        )
        if "errors" in rtn and rtn["errors"]:
            errors.append(
                "cf_merge: DsGroup.cfize_and_save: "
    ...
    with Pool(processes=n_proc) as pool:
        ...
        for dim in sorted(list(groups.keys())):
            ...
            if group.action == "split" or dim == 3:
                ...
                title = (
                    group.stem + group.name
                    if group.name not in group.stem
                    else group.stem
                )
                        ...
                        if "update_group" in result_dict:
                            ...
                            if groups[d].processed:
                                groups[d].processed = list(
                                    {*update_group["processed"], *groups[d].processed}
                                )
                            else:
                            ...
                            update_group = None
                        update_globals = (
                            result_dict["update_globals"]
                            if "update_globals" in result_dict
                            else None
                        )
                        if update_globals:
                            if "vocabulary" in update_globals:
                                [
                                    [
                                        shared["vocabulary"][v_dim][update_var].update(
                                            update_dict
                                        )
                                        for update_var, update_dict in updates.items()
                                    ]
                                    for v_dim, updates in update_globals[
                                        "vocabulary"
                                    ].items()
                                ]
                ...
                for filepath in for_controller:
                    ...
                    update_globals = (
                        r["update_globals"] if "update_globals" in r else None
                    )
                    if update_globals:
                        if "vocabulary" in update_globals:
                            [
...
def process_serial(groups: dict, shared: dict):
    ...
    for dim in sorted(list(groups.keys())):
        ...
        if group.action == "split":
            # Derive base title/filename from group's stem & dimension
            title = (
                group.stem + group.name if group.name not in group.stem else group.stem
            )
        ...
        # If group is to be merged:
            ...
            try:
                ...
                if "error" in rtn and rtn["error"]:
                    sys.exit(
                        log
                        + "\nprocess_serial: DsGroup.merge_times"
                        + str(rtn["error"])
                    )
                ...
                if "errors" in rtn and rtn["errors"]:
                    sys.exit(
                        "process_serial: DsGroup.cfize_and_save: "
                        + "; ".join([str(e) for e in rtn["errors"]])
                    )
                if "warnings" in rtn:
                ...
                if "update_group" in rtn:
                    ...
                    update_group = None
                update_globals = (
                    rtn["update_globals"] if "update_globals" in rtn else None
                )
                if update_globals:
                    if "vocabulary" in update_globals:
                        [
...
def sort_nc(directory, shared: dict) -> [dict, list]:
    ...
    for n_dim, group in shared["CONFIG"]["dimension_groups"].items():
        by_dim[n_dim] = DsGroup(
            name=group,
            n_dims=n_dim,
            action=shared["CONFIG"]["group_actions"][group],
            shared=shared,
        )
        if by_dim[n_dim].log:
    ...
    for filepath in iglob(f'{op.join(directory, "*.nc")}', recursive=False):
...
def time_units_from_input(filepaths: list) -> TimeUnits:
    ...
    if len(input_data) != 1:
        ...
        if len(input_data) > 1:
            ...
            while True:
                user_input = input(
                    "Please select number or enter units for time, including reference date in ISO format."
                )
            ...
            input_data = {
                "units": input(
                    "Enter units for time, including reference date in ISO format:"
                )
            }
    else:
    ...
    while True:
        try:
            ...
            try:
                ...
                input_data["units"] = input(
                    e + "Enter units for time, including reference date in ISO format:"
                )
                continue
...
def parse_arguments():
    ...
    parser.add_argument(
        "source_dir", help="Directory containing MONC output NetCDF files to process."
    )
    parser.add_argument(
        "--target_dir",
        "-t",
...
def cfize():
    ...
    if g["verbose"]:
        print(
            f"Process {os.getpid()}: Initialisation took "
            f"{perf_counter() - start_time} seconds."
        )

    start_time = (
        perf_counter()
    )  # wrapping in performance counter doesn't work with multiprocessing.
    ...
    if g["verbose"]:
        print(
            f"{strftime('%H:%M:%S', localtime())} "
            f"Main app process id: {os.getpid()}"
        )  # , started {start_time}
    ...
    if args.ref_time:
        ...
        g["CONFIG"]["reference_time"] and g["CONFIG"]["reference_time"] != "input_file"
    ):
    ...
    target_dir = (
        op.abspath(args.target_dir)
        if args.target_dir
        else op.abspath(
            op.join(op.dirname(source_dir), f"{op.basename(source_dir)}+processed")
        )
    )
    ...
    if not op.exists(target_dir):
        try:
            ...
            sys.exit(f"Unable to create target directory, {target_dir}. {str(e)}")
    if not os.access(target_dir, os.W_OK):
        sys.exit(
            OSError(f"Write permission denied for target directory, {target_dir}.")
        )
        ...
        if g["CONFIG"]["reference_time"] == "input_file" or (
            ...
            if g["verbose"] and time_units:
                print(
                    ...
                    "Time units found in input file:",
                    (time_units.formatted()),
                )
    ...
    try:
        if n_proc > 0:
            warnings += process_parallel(groups=group_by_dim, n_proc=n_proc, shared=g)
        else:
            warnings += process_serial(groups=group_by_dim, shared=g)
        ...
        if not args.keep_interim:
            try:
                ...
                if not g["quiet"]:
                    print(
                        strftime("%H:%M:%S", localtime()),
                        "Failed to delete interim NC files. " + str(e),
                    )
            else:
        ...
        audit_trail["processing"].update(
            {
                ...
                + str(k): {
                    ...
                    "action": "CF compliance + " + group.action
                    if group.action
                    else "CF compliance",
    ...
    for k, group in group_by_dim.items():
        print(
            ...
            + "\n           ".join([op.basename(path) for path in group.filepaths])
            + "\n          --> "
            + f"{group.action + ' --> ' if group.action and group.action != 'merge_groups' else ''}cfize -->\n           "
            + "\n           ".join([op.basename(path) for path in group.processed])
        )
    print()
    ...
    if g["verbose"]:
        print(
            f"         Main app process {os.getpid()} took "
