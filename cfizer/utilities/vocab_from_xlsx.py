...
def vocab_from_xls(filepath: str) -> dict:
    ...
    header_row = {
        c.value: i for i, c in enumerate(ws["1"]) if c.value in VOCAB_FIELDS.keys()
    }
    ...
    for i, row in enumerate(ws.rows):
        if i == 0:  # header row
            ...
            len(row[0].value) == 2
            and row[0].value[-1].upper() == "D"
            and row[0].value[0].isdigit()
    ...
    for set_name, var_set in vocab.items():
        for var, attrs in var_set.items():
            if "units" in attrs.keys():
                valid_units.update(
                    {(set_name, var): (attrs["units"], Units(attrs["units"]).isvalid)}
                )
    return valid_units
...
def vocab_to_yaml(vocab: dict) -> str:
    ...
    if op.exists(filepath):
        while True:
            overwrite = input(f"Vocabulary file {filepath} already exists. Overwrite? ")
