...
from startup import *
from utils import type_from_str, generate_coords
...
class MoncDs:
    ...
    def cfize(self, shared: dict):
        ...
        # update all (non-options-database) variables in either shared
        # ['vocabulary'][dim] or dataset:
        ...
        if not updates or 'units' not in updates:
            ...
            if var == (
                ...
                if var in self.ds.dims:
                    ...
                    # Add `positive` attribute if required/specified (not 
                    # expected).
    ...
    def cfize_variables(self, 
        ...
        for var in variables:
            if var not in self.ds.variables:
                ...
                # wildcards or use it as a stem. E.g. time_series* should 
                # identify time_series_300_1800 as a match.
