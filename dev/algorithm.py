...
def cf_merge(
    ...
    # Apply 'once per group' attributes back to resulting dataset.
    # Convert any datetime objects to strings, so xarray.Dataset.to_netcdf can
    # work with them. This can't be done until now, because finding the maximum
    # requires the correct data type.
    ...
    title = group.stem.strip('_ ') + group.n_dims if f'{group.n_dims}d' not in group.stem else group.stem.strip('_ ')
    ...
    # Call CF compliance function on dataset.
    merged = ds.cfize()  # xarray.Dataset

    # Check whether time variable name has changed
    group.time_var = ds.time_var    # Update group's time_var to match any 
                                    # update to merged dataset's.

    # Set filepath and save as processed
    filepath = op.join(target_dir, f"{merged.attrs['title']}.nc")  # f"{target_dir}/filename.nc"
    ...
    with Pool(processes=n_proc) as pool:
        ...
        for dim in sorted(list(groups.keys())):
            ...
            if group.action == 'split' or dim == 3:
                ...
                for d in set(reference_vars['dim'].values()):
                    groups[d].processed = [
